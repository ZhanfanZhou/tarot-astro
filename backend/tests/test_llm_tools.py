...
def test_named_toolsets_are_subsets():
    ...
    assert set(tools.OPENING_TOOL_NAMES) == {"submit_reading_brief", "request_user_profile",
                                             "read_divination_notes"}
    # 开场不抽牌、不取盘：牌阵由交单带出，星盘归解读 Agent
    assert "draw_tarot_cards" not in tools.OPENING_TOOL_NAMES
    assert "get_astrology_chart" not in tools.OPENING_TOOL_NAMES
    assert "draw_tarot_cards" in tools.READING_TOOL_NAMES
    # 起手单是一次性记录，解读相位不该再持有它
...
def test_specs_by_names_helper():
    ...
    assert [t["name"] for t in subset] == ["submit_reading_brief", "request_user_profile",
                                           "read_divination_notes"]
