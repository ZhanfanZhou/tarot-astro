...
async def purchase_deck(user_id: str, body: PurchaseDeckRequest):
    """用星尘解锁牌组。余额不足/不可购返回 success=false（非 HTTP 错误，便于前端温和提示）。"""
...
async def set_active_deck(user_id: str, body: SetActiveDeckRequest):
    """应用牌组到实际占卜（必须已拥有）。"""
