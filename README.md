...
手机测试：前端同时监听局域网，手机连同一个 Wi-Fi，打开终端里 Vite 打印的 `Network` 地址（形如 `http://192.168.x.x:5173`）即可，电脑上的 localhost 照常能用。开着代理 TUN 时还会多打印一个 `198.18.x.x` 的地址，那是代理的虚拟网卡，手机用不了。局域网是 http，浏览器不给剪贴板权限，「复制」按钮在手机上不可用，其余功能正常。
