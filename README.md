...
### **3）生成式大模型的应用**  
  #### 后端代码实现
  核心后端逻辑 100% 由模型生成。让模型写得准的关键不在模型，在给它的上下文：
  项目根的 `CLAUDE.md` 是一份快速地图（每个文件管什么、有哪些约定），
  `docs/superpowers/specs/` 按功能各存一份设计文档（只写现在是什么样，不记开发过程），
  `docs/TODO.md` 收还没做的事。改代码之前先读对应的那一份，
  避免模型缺少全局视角造成的漏改、错改，以及绕开已有的特殊设计。
...
## 解读一轮疑问后
    ...
    - 你可以使用`read_divination_notes`工具回顾用户的占卜历史记录，了解用户的过往经历，提供多一个解读的角度。
```
...
4. **重要！**：你可以随时使用`read_divination_notes`工具获取用户的占卜记录，回顾历史记录，了解用户的过往经历，发现重复模式，提供更有连续性和深度的解读。例如用户最近重复抽到同一张牌，你可以指出来提供；在重复以前的行为模式，也可以发现规律；还可以发现用户最近重复问相同的问题，你可以指出来；
