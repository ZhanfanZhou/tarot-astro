...
#### 3.5 塔罗牌抽牌器组件 (TarotCardDrawer.tsx)
...
**关键机制说明：**
1. **抽牌结果传递**：SYSTEM 消息在发送给 AI 时转换为带 `[抽牌结果]` 标记的 USER 消息
2. **防重复抽牌**：使用 `has_drawn_cards` 标记，后端 API 会拒绝重复抽牌请求
3. **自动触发解读**：前端自动发送触发消息，但通过 ChatMessage 组件过滤，不显示在界面上
4. **AI 识别机制**：系统提示词明确告诉 AI 看到 `[抽牌结果]` 后直接解读，不要再次返回抽牌指令
5. **继续对话**：解读完成后，用户仍可继续提问和深入探讨
...
**实现方式：**
- 对话模型包含 `has_drawn_cards` 字段
- 抽牌成功后调用 `mark_cards_drawn` 设置为 true
- 后续抽牌请求检查此字段，如果为 true 返回 400 错误
- AI 系统提示词明确说明只能抽牌一次
- AI 看到 `[抽牌结果]` 标记后不再返回抽牌指令
- 前端在解读时检测到抽牌指令会发出警告（但主要依赖后端和 AI 机制）

**三层防护：**
1. **后端 API 层**：has_drawn_cards 标记阻止重复 API 调用
2. **AI 提示层**：系统提示词和抽牌结果标记引导 AI 行为
3. **前端逻辑层**：console.warn 检测异常情况
