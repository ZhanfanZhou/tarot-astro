...
import { pngs as _pngs, thumbs as _thumbs, metas as _metas } from 'virtual:deck-assets';
...
// ── Build-time fallback ───────────────────────────────────────────────────────
// The deck images are indexed at build time so the gallery still renders when the
...
// vite-plugin-deck-assets indexes public/tarot-images/decks/ at dev/build time.
// (It replaced `import.meta.glob`s over public/ — see the plugin header.)
const _imgGlob = _pngs;

// Compressed grid thumbnails (generated by tools/gen-thumbnails.sh). Indexed
// separately so the offline fallback can pair each PNG with its .thumb.webp.
const _thumbGlob = _thumbs;

const _metaGlob = _metas as Record<string, DeckMeta>;
