...
## 2025-10-18 - 修复抽牌器选择完毕后的抽牌失败问题

**问题描述：**
在星座AI的抽牌流程中，用户选择完牌后，后端无法正确处理抽牌请求。主要表现为：
1. 前端 POST 请求到 `/api/astrology/draw` 返回 422 Unprocessable Content
2. 后端处理 `continue_with_function_result` 时出现 Gemini API ValueError

**根本原因：**

**1. Gemini Function 定义问题**
- `draw_tarot_cards` 函数的 `spread_type` 参数没有定义 enum 约束
- Gemini 返回了中文的牌阵类型描述（如 "三牌阵"）而不是英文 enum 值（如 "three_card"）
- 后端 Pydantic 模型期望 enum 值，导致验证失败，返回 422

**2. Gemini API 响应处理问题**
- 在 `continue_with_function_result` 中处理流式响应时，当 finish_reason=1（STOP）时
- Gemini API 返回的响应没有有效的 Part，访问 `.text` 属性会抛出 ValueError
- 这导致整个流式响应中断

**修复方案：**

**1. 更新 Gemini 函数定义 (backend/services/gemini_service.py)**
```python
TOOL_DRAW_TAROT_CARDS = FunctionDeclaration(
    ...
    parameters={
        "properties": {
            "spread_type": {
                "type": "string",
                "description": "牌阵类型，必须从以下选项中选择：single, three_card, celtic_cross, custom",
                "enum": ["single", "three_card", "celtic_cross", "custom"]  # ✅ 添加 enum 约束
            },
            ...
        }
    }
...
**2. 修复 Gemini 响应处理 (backend/services/gemini_service.py)**
```python
# 流式输出AI的最终响应
async for chunk in response:
    try:  # ✅ 添加 try-except 处理 ValueError
        ...
        # 当响应没有有效的 Part 时，继续处理而不中断
        print(f"[Gemini Service] 警告：处理响应时出现 ValueError: {str(e)}")
        continue
```

**3. 简化 DrawCardsRequest 模型 (backend/models.py)**
```python
class DrawCardsRequest(BaseModel):
    spread_type: str  # ✅ 改为 str，不使用 enum，提高灵活性
    card_count: int
    positions: Optional[List[str]] = None
```

**修改的文件：**
- `backend/services/gemini_service.py` - 启用 enum 约束 + 修复 ValueError 处理
- `backend/models.py` - 简化 spread_type 为 str 类型
- `frontend/src/App.tsx` - 无修改（前端逻辑已正确）

**测试验证：**
- ✅ Gemini 现在会返回正确的 enum spread_type 值
- ✅ 后端能正确解析抽牌请求（422 错误应解决）
- ✅ 流式响应不再因 ValueError 中断
- ✅ 抽牌完成后能正确保存和显示结果

**技术要点：**
1. **Gemini Function Calling** - enum 约束能引导 AI 返回预期格式
2. **Protobuf 流处理** - 需要处理没有有效 Part 的响应边界情况
3. **Type Flexibility** - 使用 str 代替 enum 提供更好的向后兼容性和灵活性
