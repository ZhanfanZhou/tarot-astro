...
**功能描述：**
为AI新增 `read_divination_notebook` 工具调用，允许AI主动读取用户的占卜笔记本，回顾历史记录，发现重复模式，提供更有连续性和深度的解读。

**核心改动：**

**1. Gemini Service 工具定义扩展**
- ✅ 新增工具 `TOOL_READ_NOTEBOOK` (read_divination_notebook)
  - 参数：`reason` - 读取笔记本的原因说明
  - 描述：详细说明工具的用途和适用场景
  - 返回：笔记本记录数量、格式化的笔记内容
- ✅ 将新工具添加到 `all_tools` 列表
- ✅ 塔罗AI和星座AI都可以使用此工具

**2. 塔罗路由处理逻辑**
- ✅ 在 `backend/routers/tarot.py` 添加 `read_divination_notebook` 处理分支
- ✅ 导入 `notebook_service`
- ✅ 实现功能：
  - 调用 `notebook_service.get_notebook(user_id)` 获取笔记本
  - 笔记本为空时返回友好提示
  - 笔记本有内容时格式化输出（时间、问题、抽到的牌、记录、用户反馈）
  - 将笔记内容作为函数结果喂回AI
  - 流式输出AI的解读
  - 保存AI的回复到对话

**3. 星座路由处理逻辑**
- ✅ 在 `backend/routers/astrology.py` 添加相同的 `read_divination_notebook` 处理逻辑
- ✅ 导入 `notebook_service`
- ✅ 与塔罗路由保持一致的处理流程

**4. 架构文档更新**
- ✅ 在 `arch.md` 添加第4个工具的完整说明
- ✅ 更新统一工具集说明（从3个更新为4个工具）
- ✅ 新增"笔记本集成"设计机制说明

**技术细节：**
- 笔记本格式化：包含时间（格式化为年月日）、问题、抽到的牌、记录摘要、用户反馈
- 时间解析：使用 `datetime.fromisoformat()` + `strftime()` 格式化显示
- 容错处理：时间解析失败时显示"未知时间"
- 打印调试信息：笔记本记录数量等关键信息
- 与现有Agent Loop流程完美集成

**使用场景：**
1. 用户主动请求："帮我看看之前的占卜记录"
2. AI主动调用：发现用户问题与历史占卜相关，自动读取回顾
3. 深度解读：结合历史记录，发现重复模式和主题演变
4. 连续性指引：基于过往经历，提供更有针对性的建议

**影响范围：**
- 后端服务：`gemini_service.py`, `tarot.py`, `astrology.py`
- 文档：`arch.md`, `update_log.md`
- 无前端改动（纯后端工具扩展）

---

## 2025-11-09 - 优化占卜笔记生成条件

**功能描述：**
优化占卜笔记生成逻辑，避免重复生成，减少生成频率，提升系统效率。

**核心改动：**

**1. NotebookEntry 数据结构扩展**
- ✅ 新增 `end_time` 字段：保存对话结束时间（对应 `conversation.updated_at`）
- ✅ 向后兼容：旧数据的 `end_time` 默认为空字符串

**2. 笔记生成条件优化**
- ✅ **条件1 - 防重复生成**：通过比对 `end_time` 和 `conversation.updated_at` 判断对话是否有新内容
  - 如果笔记已存在且 `end_time == conversation.updated_at`，说明对话无变化，不重复生成
  - 如果 `end_time != conversation.updated_at`，说明对话有新消息，可以更新笔记
- ✅ **条件2 - 延迟生成**：要求当前时间距离对话结束（`updated_at`）超过12小时
  - 避免用户刚结束对话就立即生成笔记
  - 给用户充分时间完善对话内容
  - 减少不必要的 AI 调用和成本
...
- ✅ 对话退出时打印完整的条件检查结果：
  - 基本条件（有内容、抽过牌、对话结束时间）
  - 对话是否有变化（显示上次 end_time 和当前 updated_at 对比）
  - 时间是否超过12小时（显示具体时间差）
  - 是否应该生成笔记
  - 笔记是否已更新
- ✅ 格式化输出，易于阅读和调试

**4. update_entry 返回值扩展**
- ✅ 返回详细的检查结果字典：
  - `conversation_changed`: 对话是否有变化
  - `time_elapsed`: 时间是否超过12小时
  - `should_generate`: 是否应该生成笔记
  - `existing_entry`: 是否已有笔记记录
  - `notebook_updated`: 笔记是否已更新（仅当实际生成时）
...
**技术细节：**
- 使用 `datetime.fromisoformat()` 解析时间字符串
- 使用 `timedelta(hours=12)` 计算时间差
- 错误容忍：时间解析失败时默认不满足条件
- 保持向后兼容：旧笔记数据的 `end_time` 为空时，视为对话有变化

**预期效果：**
- 避免同一对话内容多次生成笔记
- 减少12小时内的频繁笔记生成
- 降低 Gemini API 调用次数和成本
- 提升系统整体效率
