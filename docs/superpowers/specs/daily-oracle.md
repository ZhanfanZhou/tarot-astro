...
- `{history_block}` 窗口：最近 **5** 次日运记录，最远回溯 14 天（0 次时写「这是旅程的第一签」）。
  ...
  未反馈显示「未印证」；附言写入时限 30 字（超了 422，前端输入框同样限 30 字），存全文、不截断。
...
- **`DailyOracleModal`** — 顶部 14 格日历带（可回看、可印证、可跳进当日对话）+ 今日舞台
  （未抽：唤起全屏 `TarotCardDrawer`，选好一张点「确认抽牌」；已抽：牌面 + 解读 + 「继续这段对话 ›」，
  点牌面看大图）。
  点了确认到解读回来之间（`/draw` 要把整段解读生成完才返回，最长 60 秒超时）是等待态 `DrawingStage`：
  牌背上一道光来回扫 +「牌已抽出 · 正在解读」+ 一条金线在走 + 已等待秒数，
  过 15 秒说明换成「解读还在写，请留在这里稍候」；没有编造的分步进度。开减少动态效果时光和线停、秒数照跳。
  等待一开始就把舞台滚进视野（小屏上它在弹窗折线以下）。失败提示后回到未抽态，可以重抽。
  弹窗最高 88dvh（手机上 vh 比看得见的一屏高，底边会压在工具栏下）。
