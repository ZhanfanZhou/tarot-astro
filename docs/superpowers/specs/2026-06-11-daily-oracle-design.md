...
| 牌阵 | 单张牌 |
| 解读上下文 | 近 14 天日运记录(牌面+反馈)+ 用户资料,**不读对话正文** |
...
class DailyFeedback(BaseModel):
    ...
    note: Optional[str] = None                          # 一句话附言,后端截断至 50 字
...
- `{history_block}` 由 daily_service 从 `daily_draws.json` 生成,每天一行:`6月10日 | 宝剑三·逆位 | 印证:应验了 | 附言:确实和同事起了争执`;未抽的日子跳过;未反馈显示「未印证」;note 截断 50 字。**只读日运记录,不读对话正文**(聊天细节由 notebook 沉淀,仅 journey 使用)。
